...
class NewsFeed extends Component {
  ...
  shouldComponentUpdate(nextProps: any, nextState: any) {
    if (this.state.data.length < nextState.data.length) {
      return true;
    } else if (this.state.refreshing !== nextState.refreshing || this.state.loading !== nextState.loading || this.state.error !== nextState.error) {
      return true;
    }
    return false;
  }
  ...
  render() {
    return (
      <Layout menuEnabled toolbarTitle="News Feed" toolbarSubitle={this.lastUpdatedTime()}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => <EventItem onPress={this.openPage} item={item} />}
          ListFooterComponent={() => (
            <View>{this.state.error === null && <View>{this.state.ended ? <ErrorText>No more events</ErrorText> : <Loading />}</View>}</View>
          )}
