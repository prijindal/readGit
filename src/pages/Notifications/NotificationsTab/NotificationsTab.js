...
import { Text, SectionList, View, Alert, Linking } from 'react-native';
...
export default class NotificationsTab extends Component {
  ...
  updateData = async () => {
    ...
    try {
      ...
      let completeData = [...prevData, ...newData];
  ...
  clearSection = async ({ title, data }) => {
    ...
    Alert.alert('Are you sure', `${length} notifications will be cleared`, [
      {
        text: 'Cancel',
        style: 'cancel',
      },
  ...
  render() {
    return (
      <SectionList
        ...
        ListHeaderComponent={() => <View>{this.state.error !== null && <Text style={styles.error}>{this.state.error}</Text>}</View>}
        ...
        ListFooterComponent={() => (
          <View>
            {this.state.error === null && <View>{this.state.ended ? <Text style={styles.error}>No more notifications</Text> : <Loading />}</View>}
          </View>
        )}
